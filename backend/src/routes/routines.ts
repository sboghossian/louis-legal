...
routinesRouter.get("/", (req: Request, res: Response) => {
  res.json({ routines: listRoutines(res.locals.userId as string) });
});
...
routinesRouter.get("/:id", (req: Request, res: Response) => {
  const r = getRoutine(req.params.id, res.locals.userId as string);
  if (!r) { res.status(404).json({ error: "Not found" }); return; }
  res.json({ routine: r, runs: listRuns(req.params.id, res.locals.userId as string) });
});

routinesRouter.patch("/:id", (req: Request, res: Response) => {
  const updated = updateRoutine(req.params.id, res.locals.userId as string, req.body ?? {});
  if (!updated) { res.status(404).json({ error: "Not found" }); return; }
  res.json(updated);
});

routinesRouter.delete("/:id", (req: Request, res: Response) => {
  const ok = deleteRoutine(req.params.id, res.locals.userId as string);
...
routinesRouter.get("/:id/runs", (req: Request, res: Response) => {
  res.json({ runs: listRuns(req.params.id, res.locals.userId as string) });
});
