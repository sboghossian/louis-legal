/**
 * Factory tests (Wave 3 — durable sessions): picks the right backend by env,
 * and stays in-memory (safe) when Supabase is not configured.
 */
import { describe, it, expect, beforeEach, afterEach } from "vitest";

import { createSessionStore, isSupabaseConfigured } from "./factory";
...
describe("session store factory", () => {
  ...
  it("uses Supabase when both env vars are present", () => {
    ...
    expect(createSessionStore()).toBeInstanceOf(SupabaseSessionStore);
  });
