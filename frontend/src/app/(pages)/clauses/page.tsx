        ...
        <div className="flex h-full overflow-hidden">
            ...
            <div className="w-[420px] flex-shrink-0 border-r border-gray-200 flex flex-col">
                <div className="px-5 py-4 border-b border-gray-200">
                    ...
                    <div className="text-[10px] uppercase tracking-wide text-gray-500 mt-3 mb-1">Category</div>
                    ...
                    <div className="text-[10px] uppercase tracking-wide text-gray-500 mt-3 mb-1">Jurisdiction</div>
                    <div className="flex flex-wrap gap-1.5">
                    ...
                    <div className="text-[10px] uppercase tracking-wide text-gray-500 mt-3 mb-1">Language · Position</div>
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {filtered.map(c => (
                        <button
                            ...
                            className={`w-full text-left px-5 py-3 border-b border-gray-100 hover:bg-gray-50 ${selected?.id === c.id ? "bg-blue-50" : ""}`}
                        >
                            ...
                            <div className="text-[11px] text-gray-500 font-mono truncate">{c.id} · {c.jurisdictions.join(", ")}</div>
                        </button>
                    ))}
                    {!loading && filtered.length === 0 && (
                        <div className="p-6 text-sm text-gray-500">no clauses match</div>
                    )}
                </div>
            </div>
                ...
                {!selected ? (
                    <div className="p-12 text-sm text-gray-500">
                        ...
                        <div className="mt-4 text-xs">
                            ...
                            <div>Backend: <code className="bg-gray-100 px-1 rounded">{API_BASE}/api/clauses</code></div>
                        </div>
                    </div>
                ) : (
                    <div className={`flex ${comparing ? "divide-x divide-gray-200" : ""}`}>
                        <ClausePane
                            clause={selected}
                            comparing={!!comparing}
    ...
    return (
        <div className={`${comparing ? "w-1/2" : "w-full"} p-8 max-w-3xl`}>
            <div className="flex items-start justify-between mb-1">
                <div className="text-xs text-gray-500 font-mono">{clause.id}</div>
                {isCompare && (
            ...
            <div className="flex flex-wrap gap-1.5 mb-4 text-xs">
                ...
                {clause.jurisdictions.map(j => (
                    <span key={j} className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded">{j}</span>
                ))}
            </div>

            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-sm whitespace-pre-wrap font-serif mb-4" dir={clause.language === "ar" ? "rtl" : "ltr"}>
                {clause.body}
            </div>
            ...
            {clause.notes && (
                <div className="mb-4">
                    <div className="text-xs font-semibold uppercase tracking-wide text-gray-600 mb-1">Drafting notes</div>
                    <div className="text-sm text-gray-700 bg-blue-50/40 border border-blue-100 rounded p-3 whitespace-pre-wrap">{clause.notes}</div>
                </div>
            )}
            ...
            {clause.risk_flags && clause.risk_flags.length > 0 && (
                <div className="mb-4">
                    ...
                    <ul className="text-sm text-gray-700 bg-amber-50/40 border border-amber-200 rounded p-3 space-y-1.5 list-disc list-inside">
            ...
            {clause.alternates && clause.alternates.length > 0 && (
                <div className="mb-4">
                    <div className="text-xs font-semibold uppercase tracking-wide text-gray-600 mb-1">Alternates</div>
                    <div className="flex flex-wrap gap-1.5">
                        {clause.alternates.map(altId => (
                            <div key={altId} className="inline-flex items-center bg-gray-100 rounded text-xs">
                                <button
                                    onClick={() => onAlternate(altId)}
                                    className="px-2 py-1 hover:bg-gray-200 rounded-l font-mono"
                                >
                                    {altId}
                                </button>
                                {showCompareButton && (
                                    <button
                                        ...
                                        className="px-2 py-1 hover:bg-gray-200 rounded-r border-l border-gray-200"
                                    >
