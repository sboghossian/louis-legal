    ...
    return (
        <div className="flex h-full overflow-hidden">
            ...
            <div className="w-[380px] flex-shrink-0 border-r border-border flex flex-col">
                <div className="px-5 py-4 border-b border-border">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <h1 className="text-lg font-semibold">{t("legal_flows.title")}</h1>
                    </div>
                    <div className="flex gap-1.5">
                        <FilterChip active={view === "library"} onClick={() => setView("library")}>
                            {t("legal_flows.tab.library")} {templates.length > 0 && `· ${templates.length}`}
                        </FilterChip>
                        <FilterChip active={view === "runs"} onClick={() => setView("runs")}>
                            {t("legal_flows.tab.runs")} {runs.length > 0 && `· ${runs.length}`}
                        </FilterChip>
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto">
                    {loading && <div className="p-6 text-sm text-muted-foreground">{t("common.loading")}</div>}
                    {view === "library" && templates.map(tpl => (
                        <button
                            key={tpl.id}
                            onClick={() => openTemplate(tpl.id)}
                            className={`w-full text-left px-5 py-3 border-b border-border hover:bg-muted ${selectedTpl?.id === tpl.id ? "bg-blue-50" : ""}`}
                        >
                            <div className="flex items-center gap-2 mb-1">
                                <Badge variant="secondary" className="text-[10px]">{tpl.category}</Badge>
                                <span className="text-[10px] text-muted-foreground">{t("legal_flows.steps_count", { count: tpl.stepCount })}</span>
                            </div>
                            <div className="text-sm font-medium text-foreground">{tpl.title}</div>
                            <div className="text-xs text-muted-foreground line-clamp-2 mt-1">{tpl.description}</div>
                            <div className="text-[10px] text-muted-foreground mt-1">{tpl.jurisdictions.join(", ")}</div>
                        </button>
                    ))}
                    {view === "runs" && runs.map(r => {
                        const tpl = templates.find(tpl2 => tpl2.id === r.flowId);
                        return (
                            <button
                                key={r.id}
                            ...
                            >
                                <div className="flex items-center gap-2 mb-1">
                                    <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${STATUS_STYLE[r.status]}`}>
                                        {t(`legal_flows.status.${r.status}`)}
                                    </span>
                                    <span className="text-[10px] text-muted-foreground">
                                        {t("legal_flows.step_progress", { done: r.stepHistory.filter(s => s.completedAt).length, total: tpl?.stepCount ?? "?" })}
                                    </span>
                                </div>
                    ...
                    {view === "runs" && !loading && runs.length === 0 && (
                        <div className="p-6 text-sm text-muted-foreground">{t("legal_flows.empty.no_runs")}</div>
                    )}
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {view === "library" && !selectedTpl && (
                    <div className="p-12 text-sm text-muted-foreground">{t("legal_flows.empty.library")}</div>
                )}
                ...
                {view === "runs" && !selectedRun && (
                    <div className="p-12 text-sm text-muted-foreground">{t("legal_flows.empty.runs")}</div>
                )}
            </div>
        </div>
    );
...
function TemplateView({ template, onStart }: { template: FlowTemplate; onStart: () => void }) {
    ...
    return (
        <div className="p-8 max-w-3xl">
            ...
            <div className="text-xs text-muted-foreground mb-6">{t("legal_flows.jurisdictions", { list: template.jurisdictions.join(", ") })}</div>

            <Button onClick={onStart} className="mb-6">
                <Play className="w-3.5 h-3.5 mr-1" />
                {t("legal_flows.start_run")}
            </Button>

            <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground mb-3">{t("legal_flows.steps_header", { count: template.steps.length })}</h2>
