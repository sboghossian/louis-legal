                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {filtered.map(e => (
                        <button
                            ...
                            className={`w-full text-left px-5 py-3 border-b border-gray-100 hover:bg-gray-50 ${selected?.frontmatter.id === e.id ? "bg-blue-50" : ""}`}
                        >
                            ...
                            <div className="text-xs text-gray-500 font-mono truncate">{e.id}</div>
                        </button>
                    ))}
                    {!loading && filtered.length === 0 && (
                        <div className="p-6 text-sm text-gray-500">no skills match</div>
                    )}
                </div>
            </div>
                ...
                {!selected ? (
                    <div className="p-12 text-sm text-gray-500">
                        ...
                        <div className="mt-4 text-xs">
                            ...
                            <div>Backend: <code className="bg-gray-100 px-1 rounded">{API_BASE}/api/skills</code></div>
                        </div>
                    </div>
                ) : (
                    <div className="p-8 max-w-3xl">
                        <div className="text-xs text-gray-500 font-mono mb-1">{selected.frontmatter.id}</div>
                            ...
                            {Object.entries(selected.frontmatter).map(([k, v]) => {
                                ...
                                return (
                                    <span key={k} className="bg-gray-100 px-2 py-1 rounded">
                                        <span className="text-gray-500">{k}:</span> <span className="text-gray-800">{display}</span>
                                    </span>
                                );
                            })}
                        </div>
                        <div className="prose prose-sm max-w-none whitespace-pre-wrap font-mono text-xs bg-gray-50 border border-gray-200 rounded-lg p-4">
                            {selected.prompt}
                        </div>
    ...
    return (
        <div className="max-w-5xl mx-auto px-8 py-6">
            ...
            {stats && (
                <div className="grid grid-cols-3 gap-4 mb-8">
                    <div className="border border-gray-200 rounded-lg p-4">
                        <div className="text-xs text-gray-500 uppercase tracking-wide">Total decisions</div>
                        <div className="text-2xl font-semibold mt-1">{stats.total}</div>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-4">
                        <div className="text-xs text-gray-500 uppercase tracking-wide mb-2">By intent</div>
                        <div className="space-y-1">
                            {Object.entries(stats.byIntent).sort((a,b) => b[1]-a[1]).map(([k,v]) => (
                                <div key={k} className="flex justify-between text-xs"><span className="text-gray-700">{k}</span><span className="text-gray-500">{v}</span></div>
                            ))}
                        </div>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-4">
                        <div className="text-xs text-gray-500 uppercase tracking-wide mb-2">By classifier</div>
                        <div className="space-y-1">
            ...
            <div className="border border-gray-200 rounded-lg p-4 mb-8">
                <div className="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2">Test the router</div>
                <textarea
                    value={testMessage}
                ...
                {testResult && (
                    <div className="mt-3 text-xs text-gray-600">
                        <span className="text-gray-500">skills loaded: </span>
            ...
            {/* Decisions list */}
            <div className="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-3">Recent decisions ({decisions.length})</div>
                ...
                {decisions.map((d, i) => (
                    <div key={i} className="px-4 py-3">
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-[10px] font-mono text-gray-500">{formatTs(d.ts)}</span>
                            <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${SRC_COLOR[d.classifierSource] || "bg-gray-100 text-gray-600"}`}>
                                {d.classifierSource}
                            </span>
                            <span className="text-xs font-semibold">{d.intentPrimary}</span>
                            {d.intentPracticeArea && <span className="text-[10px] text-gray-500">· {d.intentPracticeArea}</span>}
                            {d.intentJurisdiction && <span className="text-[10px] text-gray-500">· {d.intentJurisdiction}</span>}
                            <span className="ml-auto text-[10px] text-gray-500">
                                {d.skillCount} skills · {d.systemPromptChars} chars{d.latencyMs != null ? ` · ${d.latencyMs}ms` : ""}
                            </span>
                        </div>
                        <div className="text-xs text-gray-700 italic truncate">{d.messagePreview}</div>
                        <div className="text-[10px] text-gray-400 font-mono mt-1 truncate">{d.skillIds.slice(0, 6).join(", ")}{d.skillIds.length > 6 ? ` (+${d.skillIds.length - 6})` : ""}</div>
                    </div>
                ))}
            </div>
        </div>
