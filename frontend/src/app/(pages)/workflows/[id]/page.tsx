...
import { getWorkflow, updateWorkflow } from "@/app/lib/louisApi";
...
import type { ColumnConfig, LouisWorkflow } from "@/app/components/shared/types";
import {
    BUILT_IN_IDS,
    BUILT_IN_WORKFLOWS,
...
export default function WorkflowDetailPage({ params }: Props) {
    ...
    const [workflow, setWorkflow] = useState<LouisWorkflow | null>(null);
    ...
    if (notFound || !workflow) {
        return (
            <div className="flex-1 flex items-center justify-center">
                <p className="text-muted-foreground font-serif">Workflow not found.</p>
            </div>
        );
    }
                            ...
                            {columns.length === 0 ? (
                                ...
                                columns.map((col) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div
                                                className={`sticky left-0 z-[60] ${CHECK_W} p-2 flex items-center justify-center ${isChecked ? "bg-muted" : "bg-card"} group-hover:bg-muted`}
                                            ...
                                            <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${isChecked ? "bg-muted" : "bg-card"} group-hover:bg-muted`}>
