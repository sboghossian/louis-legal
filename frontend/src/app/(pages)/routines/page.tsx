                    ...
                    return (
                        <div key={r.id} className={`border rounded-lg p-4 ${r.enabled ? "border-gray-200 bg-white" : "border-gray-200 bg-gray-50 opacity-75"}`}>
                            <div className="flex items-start gap-3 mb-2">
                                <Repeat className="w-5 h-5 text-gray-700 mt-0.5" />
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1.5">
                                        <span className="font-medium text-sm truncate">{r.title}</span>
                                        <span className="text-[10px] uppercase tracking-wide text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{KIND_LABEL[r.kind]}</span>
                                    </div>
                            ...
                            <div className="flex justify-between text-[10px] text-gray-400 mt-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => openRuns(r)} className="text-gray-600 hover:underline">Runs</button>
                ...
                <div className="overflow-y-auto px-6 py-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select value={kind} onChange={e => setKind(e.target.value as Routine["kind"])} className="mt-1 w-full border border-gray-300 rounded px-3 py-2 text-sm">
                                {Object.entries(KIND_LABEL).map(([k, label]) => <option key={k} value={k}>{label}</option>)}
                            </select>
                        </div>
                        <div>
                            <Label className="text-xs">Output</Label>
                            <select value={outputChannel} onChange={e => setOutputChannel(e.target.value as Routine["outputChannel"])} className="mt-1 w-full border border-gray-300 rounded px-3 py-2 text-sm">
            ...
            <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
                <div className="px-6 py-4 border-b flex items-center justify-between">
                    <div>
                        ...
                        <div className="text-xs text-gray-500 mt-0.5">{routine.title}</div>
                    </div>
                    <Button variant="ghost" size="sm" onClick={onClose}><X className="w-4 h-4" /></Button>
                </div>
                <div className="overflow-y-auto px-6 py-4 space-y-2">
                    {runs.length === 0 && <div className="text-sm text-gray-500 italic">No runs yet. Click "Run now" to trigger one.</div>}
                    {runs.map(run => (
                        <div key={run.id} className="border rounded p-3 text-sm">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs text-gray-500">started {new Date(run.startedAt).toLocaleString()}</span>
                            </div>
                            {run.output && <div className="text-xs whitespace-pre-wrap bg-gray-50 p-2 rounded font-mono">{run.output}</div>}
