...
const CATEGORY_META: Record<Category, { labelKey: string; icon: React.ComponentType<{ className?: string }>; accent: string }> = {
    "legal-tool":   { labelKey: "integrations.category.legal_tool",    icon: Building2,    accent: "bg-blue-50 text-blue-700" },
    research:       { labelKey: "integrations.category.research",      icon: Brain,        accent: "bg-emerald-50 text-emerald-700" },
    productivity:   { labelKey: "integrations.category.productivity",  icon: Briefcase,    accent: "bg-purple-50 text-purple-700" },
    communication:  { labelKey: "integrations.category.communication", icon: MessageSquare, accent: "bg-amber-50 text-amber-700" },
    billing:        { labelKey: "integrations.category.billing",       icon: DollarSign,   accent: "bg-rose-50 text-rose-700" },
    "ai-mcp":       { labelKey: "integrations.category.ai_mcp",        icon: Brain,        accent: "bg-indigo-50 text-indigo-700" },
    developer:      { labelKey: "integrations.category.developer",     icon: Code,         accent: "bg-muted text-foreground/80" },
};
...
export default function IntegrationsPage() {
    ...
    async function disconnect(integration: Integration) {
        const ok = await confirm({
            title: t("integrations.disconnect.title", { name: integration.name }),
            message: t("integrations.disconnect.message"),
    ...
    return (
        <div className="max-w-6xl mx-auto px-8 py-8">
            <div className="flex items-center gap-2 mb-2">
                ...
                <Badge variant="secondary">{t("integrations.count", { connected: connectedCount, total: all.length })}</Badge>
            </div>
            ...
            <div className="flex gap-3 mb-4">
                <div className="relative flex-1">
                    ...
                    <Input value={q} onChange={e => setQ(e.target.value)} placeholder={t("integrations.search_placeholder")} className="pl-9" />
                </div>
                <label className="flex items-center gap-2 px-3 border border-border rounded-lg text-sm cursor-pointer">
                    ...
                    {t("integrations.only_connected")}
                </label>
            </div>

            <div className="flex flex-wrap gap-1.5 mb-6">
                <Chip active={activeCategory === null} onClick={() => setActiveCategory(null)}>{t("integrations.category.all")}</Chip>
            ...
            <div className="mt-8 text-xs text-muted-foreground border-t border-border pt-6">
                <p>
                    {t("integrations.footer.before")}
                    <a href="/docs" className="text-blue-600 underline">{t("integrations.footer.link")}</a>.
                </p>
            </div>
        </div>
    );
...
function IntegrationCard({
    ...
    const statusLabel = t(`integrations.status.${i.status.replace(/-/g, "_")}`);
    return (
        <div className={`border rounded-lg p-4 flex flex-col ${connected ? "border-green-300 bg-green-50/30" : "border-border bg-card"}`}>
            <div className="flex items-start gap-2 mb-1">
                <div className="flex-1 min-w-0">
                    ...
                    <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${STATUS_STYLE[i.status]} mt-0.5`}>{statusLabel}</span>
                </div>
                {i.signupUrl && (
            ...
            <div className="mt-auto flex items-center gap-2">
                {i.status === "coming-soon" ? (
                    ...
                    <>
                        ...
                        <Button size="sm" variant="ghost" className="h-7 text-xs text-red-600" onClick={onDisconnect} title={t("action.disconnect")}>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-lg w-full max-w-md">
                <div className="px-6 py-4 border-b flex items-center justify-between">
                    <div>
                        <h2 className="font-semibold">{t("integrations.connect.title", { name: integration.name })}</h2>
                ...
                <div className="px-6 py-4 space-y-3">
                    {integration.authType === "oauth" && (
                        <div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm">
                            <p className="font-medium mb-1">{t("integrations.oauth.title")}</p>
                            <p className="text-xs text-blue-800">
                                {t("integrations.oauth.intro", { name: integration.name })}
                            </p>
                        </div>
                    )}
                    {integration.authType === "api-key" && (
                        <div>
                            <Label className="text-xs">{t("integrations.api_key")}</Label>
                    ...
                    {integration.authType === "url" && (
                        <div>
                            <Label className="text-xs">{t("integrations.server_url")}</Label>
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1">{t("integrations.mcp_note")}</p>
                        </div>
                    )}
                    {integration.id === "openclaw" && (
                        <div>
                            <Label className="text-xs">{t("integrations.openclaw.workspace")}</Label>
                            <Input value={workspaceId} onChange={e => setWorkspaceId(e.target.value)} placeholder="haqq-beirut" className="mt-1" />
                        </div>
                    )}
                    {integration.id === "github" && (
                        <div>
                            <Label className="text-xs">{t("integrations.github.repo")}</Label>
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1">{t("integrations.github.repo_note")}</p>
                        </div>
                    )}
                    {integration.signupUrl && (
                        <p className="text-[10px] text-muted-foreground">
                            {t("integrations.signup_note", { name: integration.name })}
                        </p>
                    )}
                </div>
                <div className="px-6 py-4 border-t flex justify-end gap-2">
                    ...
                    <Button onClick={submit} disabled={submitting || (integration.authType === "api-key" && !apiKey.trim())}>
                        {submitting ? t("action.connecting") : <><Check className="w-3.5 h-3.5 mr-1" /> {t("action.connect")}</>}
                    </Button>
                </div>
            </div>
        </div>
