...
function BulkEditActions({
    ...
    onViewClick?: (ann: LouisEditAnnotation, filename: string) => void;
    onResolveStart?: (args: {
        editId: string;
        documentId: string;
        verb: "accept" | "reject";
    ...
    return (
        <div className="w-full h-9 flex items-center mb-2">
            <LouisIcon
                ...
                error={isError}
                louis={!isError && !(showDone && doneVisible)}
                size={22}
            />
        </div>
    );
...
function preprocessCitations(
    ...
    citationsList: LouisCitationAnnotation[],
): string {
...
function MarkdownContent({
    ...
    text: string;
    citationsList: LouisCitationAnnotation[];
    onCitationClick?: (c: LouisCitationAnnotation) => void;
...
interface Props {
    ...
    isStreaming?: boolean;
    isError?: boolean;
    /** Human-readable error text rendered alongside the red Louis icon. */
    errorMessage?: string;
    annotations?: LouisCitationAnnotation[];
    onCitationClick?: (citation: LouisCitationAnnotation) => void;
    minHeight?: string;
    onWorkflowClick?: (workflowId: string) => void;
    onEditViewClick?: (ann: LouisEditAnnotation, filename: string) => void;
    /**
...
export function AssistantMessage({
    ...
    const citationsList: LouisCitationAnnotation[] = [];
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        {!isStreaming &&
                            (() => {
                                ...
                                const statusOf = (ann: LouisEditAnnotation) =>
