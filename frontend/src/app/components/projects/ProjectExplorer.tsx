...
import type { LouisDocument, LouisFolder } from "@/app/components/shared/types";
...
interface Props {
    ...
    onDocClick: (doc: LouisDocument) => void;
...
export function ProjectExplorer({
    ...
    async function handleDropOnTarget(targetFolderId: string | null, e: React.DragEvent) {
        ...
        const movingFolderId = e.dataTransfer.getData("application/louis-folder");
    ...
    function isInternalDrag(e: React.DragEvent): boolean {
        return (
            Array.from(e.dataTransfer.types).includes("application/louis-doc") ||
            Array.from(e.dataTransfer.types).includes("application/louis-folder")
        );
    }
                ...
                {creatingIn === parentId && (
                    ...
                    >
                        ...
                        <input
                            ...
                            autoFocus
                            className="flex-1 min-w-0 text-xs bg-transparent outline-none border-b border-border text-foreground"
    ...
    return (
        ...
        >
            ...
            {documents.length === 0 && folders.length === 0 && creatingIn === undefined && (
                <li className="px-4 py-2 text-xs text-muted-foreground">No documents in this project.</li>
            )}
            ...
            {contextMenu && (
                <div
                    ref={contextMenuRef}
                    className="fixed z-50 w-44 rounded-lg border border-border bg-card shadow-lg overflow-hidden text-xs"
                    style={{ top: contextMenu.y, left: contextMenu.x }}
                >
                    {onCreateFolder && (
                        <button
                            className="w-full px-3 py-1.5 text-left text-foreground/80 hover:bg-muted flex items-center gap-2"
                            onClick={() => {
                                setContextMenu(null);
                    ...
                    {contextMenu.folderId && onRenameFolder && (
                        <button
                            className="w-full px-3 py-1.5 text-left text-foreground/80 hover:bg-muted"
                            onClick={() => {
