  ...
  return (
    <div className={`${FRAME} ${className ?? ""}`}>
      ...
      >
        ...
        <g
          className="louis-rise"
          style={{ animationDelay: "0.45s" } as React.CSSProperties}
        >
          <rect
            x="140"
            y="135"
    ...
    <div className={`${FRAME} ${className ?? ""}`}>
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5">
        <svg viewBox="0 0 64 64" className="w-12 h-12">
          ...
          />
          <path
            d="M 26 26 V 22 A 6 6 0 0 1 38 22 V 26"
            fill="none"
            stroke="#1f2937"
            strokeWidth="1.2"
          />
        ...
        <div className="text-[7.5px] text-gray-600 font-medium">
          AES-256 · privilege-aware
        </div>
        <div className="grid grid-cols-3 gap-1 text-[6.5px] text-gray-500 mt-1">
          {["encrypted at rest", "per-client isolation", "audit log"].map(
            (t) => (
              <div
                key={t}
                className="rounded border border-gray-200 bg-white px-1 py-0.5 text-center"
              >
                {t}
              </div>
            ),
      ...
      <div className="absolute inset-3 grid grid-cols-3 gap-1 text-[7px]">
        {personas.map((p, i) => (
          ...
          >
            ...
            <div className="text-gray-800 leading-tight text-center truncate w-full">
              {p.name}
            </div>
          </div>
        ))}
    ...
    <div className={`${FRAME} ${className ?? ""}`}>
      <div className="absolute inset-3 flex flex-col gap-1 text-[7px]">
        ...
        {models.map((m, i) => (
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${m.active ? "bg-emerald-400 louis-pulse" : "bg-gray-300"}`}
            />
            <div className="flex-1 min-w-0">
              <div className="truncate font-medium">{m.name}</div>
              <div
                className={`text-[6px] ${m.active ? "text-gray-300" : "text-gray-400"}`}
              >
                {m.provider}
              </div>
            </div>
            <div
              className={`text-[6px] px-1 rounded ${m.active ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}
            >
              {m.status}
            </div>
          </div>
    ...
    <div className={`${FRAME} ${className ?? ""}`}>
      <div className="absolute inset-3 flex flex-col gap-1 text-[7px]">
        <div className="rounded-md border border-gray-200 bg-white px-2 py-1 flex items-center gap-1.5 text-gray-600">
          <svg viewBox="0 0 16 16" className="w-2 h-2">
            <circle
              cx="7"
              cy="7"
              r="5"
          ...
          <span className="italic">
            Ninth Circuit on qualified immunity 2023
          </span>
        </div>
        {cases.map((c, i) => (
          <div
          ...
          >
            <div className="flex items-center gap-1 mb-0.5">
              ...
              <div className="text-gray-900 font-medium italic truncate">
                {c.name}
              </div>
            </div>
...
export function ApprovalAuditMock({ className }: MockProps) {
  return (
    <div className={`${FRAME} ${className ?? ""}`}>
      <div className="absolute inset-3 flex flex-col gap-1.5 text-[7px]">
        <div className="text-[6.5px] uppercase tracking-wider text-amber-700">
          Pending approvals · 2
        </div>
        <div className="rounded-md border border-amber-200 bg-amber-50/60 px-2 py-1 louis-rise">
          <div className="flex items-center gap-1 mb-0.5">
            <span className="px-1 rounded bg-amber-200 text-amber-900 text-[6px] font-medium">
              REVIEW
            </span>
            <div className="text-gray-800 truncate flex-1">
              Send MNDA to globex@acme.com
            </div>
          </div>
          <div className="flex items-center gap-1 text-[6px] text-gray-500">
            <span>requested by agent · 2m ago</span>
            <span className="ml-auto px-1 rounded bg-emerald-600 text-white">
              Approve
            </span>
            <span className="px-1 rounded bg-gray-100 text-gray-700">Hold</span>
          </div>
        </div>
        <div className="text-[6.5px] uppercase tracking-wider text-gray-400 mt-1">
          Audit log · today
        </div>
        {[
    ...
    <div className={`${FRAME} ${className ?? ""}`}>
      <div className="absolute inset-3 flex flex-col gap-1.5">
        <div className="flex items-center gap-1 text-[6.5px] text-gray-500">
          <span className="px-1 rounded bg-rose-100 text-rose-700 border border-rose-200">
            v6
          </span>
          <span>→</span>
          <span className="px-1 rounded bg-emerald-100 text-emerald-700 border border-emerald-200">
            v7
          </span>
        ...
        <div className="grid grid-cols-2 gap-1.5 flex-1 text-[6.5px] font-serif">
          <div className="rounded-md border border-gray-200 bg-white p-1.5 leading-relaxed">
            <div className="font-semibold text-[7px] mb-0.5">
              §9.4 Survivor cap
            </div>
            <div className="text-gray-700">
              Survivor cap shall be{" "}
              <span className="bg-rose-100 text-rose-700 line-through px-0.5">
                $5M
              </span>{" "}
              for{" "}
              <span className="bg-rose-100 text-rose-700 line-through px-0.5">
          ...
          <div className="rounded-md border border-gray-200 bg-white p-1.5 leading-relaxed">
            ...
            <div className="text-gray-700">
              Survivor cap shall be{" "}
              <span className="bg-emerald-100 text-emerald-800 px-0.5 louis-pulse">
                $1.2M
              </span>{" "}
              for{" "}
              <span className="bg-emerald-100 text-emerald-800 px-0.5">
                18 months
              </span>
              .
            </div>
        ...
        <div className="text-[6.5px] text-amber-800 italic">
          Agent summary: cap and tail both reduced — conflicts with §9.6
          fundamental reps.
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 h-7 border-b border-[#e7e2d6] bg-white/70 backdrop-blur flex items-center px-3 gap-1.5">
        ...
        <div className="ml-3 text-[10px] text-gray-500 font-mono">
          legal.dashable.dev / assistant
        </div>
      </div>
      ...
      <div className="absolute inset-x-0 top-7 bottom-0 flex">
        ...
        <div className="flex-1 p-3 flex flex-col gap-2 text-[9px]">
          <div className="self-end max-w-[70%] rounded-lg rounded-br-sm bg-gray-900 text-white px-2.5 py-1.5">
            Review the Globex SPA for hidden indemnity caps + flag any unusual
            MAC carve-outs.
          </div>
          ...
          <div className="self-start max-w-[85%] rounded-lg rounded-bl-sm bg-white border border-gray-200 px-2.5 py-1.5 text-gray-800 shadow-sm">
            ...
            <div>
              ...
              after 18 months, which conflicts with the unconditional
              fundamental-reps carve-out in §9.6.
              <span className="louis-caret">▎</span>
            </div>
